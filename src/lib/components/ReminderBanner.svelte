<script>
  ...
  $: message = reminderType
    ? ReminderSystem.getReminderMessage(reminderType)
    : "";

  async function handleLater() {
    ReminderSystem.dismissReminder("later");
    // Ensure the dismissal is recorded before unmounting
    await tick();
...
<div
  ...
  style="background: rgba(59,130,246,0.15); border: 1px solid rgba(59,130,246,0.3); padding: 1rem; border-radius: 18px; margin-bottom: 1rem;"
>
  <div style="display: flex; align-items: start; gap: 0.75rem;">
    <div style="font-size: 1.5rem; flex-shrink: 0;">💾</div>
    <div style="flex: 1;">
      <p
        style="margin: 0 0 0.75rem 0; color: rgba(255,255,255,0.9); font-size: 0.875rem;"
      >
        {message}
      </p>
